...
static rt_err_t SCL_H(struct rt_i2c_bit_ops *ops)
{
    ...
    {
        LOG_I("wait %ld tick for SCL line to go high",
              rt_tick_get() - start);
    }
#endif
...
static rt_int32_t i2c_readb(struct rt_i2c_bus_device *bus)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (SCL_H(ops) < 0)
        {
            LOG_I("i2c_readb: wait scl pin high "
                                     ...
                                     struct rt_i2c_msg        *msg)
{
    ...
    if (flags & RT_I2C_ADDR_10BIT)
    {
        ...
        LOG_I("addr1: %d, addr2: %d", addr1, addr2);
        ...
        if (flags & RT_I2C_RD)
        {
            LOG_I("send repeated start condition");
            i2c_restart(ops);
            addr1 |= 0x01;
            ret = i2c_send_address(bus, addr1, retries);
            if ((ret != 1) && !ignore_nack)
                              ...
                              rt_uint32_t               num)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & RT_I2C_NO_START))
        {
            ...
            {
                LOG_I("receive NACK from device addr 0x%02x msg %d\r\n",
                        msgs[i].addr, i);
                goto out;
            }
        }
        if (msg->flags & RT_I2C_RD)
        {
            ...
            if (ret >= 1)
                LOG_I("read %d byte%s", ret, ret == 1 ? "" : "s");
            if (ret < msg->len)
            {
                if (ret >= 0)
                    ret = -RT_EIO;
        ...
        else
        {
            ...
            if (ret >= 1)
                LOG_I("write %d byte%s", ret, ret == 1 ? "" : "s");
            if (ret < msg->len)
            {
                if (ret >= 0)
                    ret = -RT_ERROR;
