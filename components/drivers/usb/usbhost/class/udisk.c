...
//        pindex  要获取的分区表的索引
        ...
        if (ret == RT_EOK)
        {
            ...
            stor->dev_cnt++;
            //add 2020.5
//            result=dfs_mkfs("elm", stor->dev[i].parent.name);
//                   if (result != RT_EOK)
//                      {
//                          rt_kprintf("mkfs failed, result=%d\n", result);
//                      }else {
//                          rt_kprintf("mkfs ok, result=%d\n", result);
//                      }
            ...
            if (dfs_mount(stor->dev[i].parent.name, UDISK_MOUNTPOINT, "elm",
                0, 0) == 0)
            {
                rt_event_send(&event_main,EVENT_USBH);
              F4_status.udisk_inited=RT_TRUE;
                RT_DEBUG_LOG(1, ("udisk  part %d mount successfully\n", i));//RT_DEBUG_USB
            }
            else
            {
                RT_DEBUG_LOG(1, ("udisk part %d mount failed\n", i));
            }
        }
        else
        {
            if(i == 0)
            {
                ...
                else
                {
                    ...
                    led_name_b_r_onoff(1, "开关", 0, 1); //控灯状态
                    irq_vbs_en(0);
