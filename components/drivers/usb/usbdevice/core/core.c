...
static rt_size_t rt_usbd_ep_write(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
    ...
    if (ep->ep_desc->bEndpointAddress==0x83) {
              volatile char a=0;
          }else    if (ep->ep_desc->bEndpointAddress==0x3) {
              volatile char a=0;
          }
...
rt_size_t rt_usbd_io_request(udevice_t device, uep_t ep, uio_request_t req)
{
    ...
    if (RT_DEBUG_USB) rt_kprintf( "usbd_io_request epaddr: %x\n",ep->ep_desc->bEndpointAddress);
    ...
    if(ep->stalled == RT_FALSE)
    {
        ...
        case UIO_REQUEST_READ_FULL:
            ...
            if (RT_DEBUG_USB)
                rt_kprintf( "UIO_REQUEST_READ_FULL  buff:%x,size:%x\n",req->buffer,req->size);
            break;
        case UIO_REQUEST_WRITE:
            ...
            if (RT_DEBUG_USB)   rt_kprintf( "UIO_REQUEST_WRITE  buff:%x,size:%x\n",req->buffer,req->size);
        ...
        if (RT_DEBUG_USB) {
             rt_kprintf( "usbd_io_request req_type %d\n",req->req_type);
        }
...
static void rt_usbd_thread_entry(void* parameter)
{
    while(1)
    {
      ...
      //  RT_DEBUG_LOG(RT_DEBUG_USB, ("thread message type %d\n", msg.type));//RT_DEBUG_USB
   //    RT_DEBUG_LOG(1, ("thread message type %d\n", msg.type));//RT_DEBUG_USB
        switch (msg.type)
        {
        case USB_MSG_SOF:
            _sof_notify(device);
...
//            }
        ...
        case USB_MSG_SETUP_NOTIFY:
           ...
           // LOG_I("charge\r\n\n");
            F4_status.conmuter_connected=1;//连USB
            break;
        ...
        case USB_MSG_RESET:
         //   rt_pm_run_enter(PM_RUN_MODE_HIGH_SPEED);
            ...
            rt_event_send(&event_main,EVENT_USBD_Reset);//usb插入
            break;
        ...
        case USB_MSG_PLUG_OUT:
            rt_event_send(&event_main,EVENT_USBD_DISCONNECT);
            device->state = USB_STATE_NOTATTACHED;    
            _stop_notify(device);
            break;
        default:
