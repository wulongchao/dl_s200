...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size)
{
    ...
    RT_ASSERT(func != RT_NULL);  //bufflag

    data = (struct vcom*)func->user_data;
//    if (data->ep_in->buffer==RT_NULL) {
//          data->ep_in->buffer=malloc(4096);
//      }
    request_size = data->ep_in->request.size;
    RT_DEBUG_LOG(RT_DEBUG_USB, ("cdc _ep_in_handler %d\n", request_size));
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("cdc _ep_out_handler %d\n", size));
...
static void rt_usb_vcom_init(struct ufunction *func)
{
    ...
    data->serial.serial_rx  = RT_NULL;//rt_malloc(CDC_RX_BUFSIZE);//
