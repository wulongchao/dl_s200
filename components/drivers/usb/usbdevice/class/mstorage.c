...
static rt_size_t _read_capacity(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    data = (struct mstorage*)func->user_data;   
    buf = data->ep_in->buffer;    
    //data->geometry.sector_count=256*6;
    sector_count = data->geometry.sector_count;
    sector_size = data->geometry.bytes_per_sector;
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("msg _ep_in_handler"));
    data = (struct mstorage*)func->user_data;
    ...
    case STAT_CSW:
     ...
     case STAT_SEND:        
        ...
        {
            unsigned long int addr=data->block+(USB_FLASH_START_ADDR/4096);
//            rt_kprintf("\nread size %d block 0x%x oount 0x%x\n",
//                                                     size, addr, data->size);
            if(rt_device_read(data->disk, addr,data->ep_in->buffer, 1) == 0)
            {
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    if(data->status == STAT_CBW)
    {
        ...
        RT_DEBUG_LOG(RT_DEBUG_USB, ("_ep_out_handler cmd %d\r\n",cmd->cmd));
...
//                                          size, addr, data->size);
        ...
        rt_device_write(data->disk,addr,data->ep_out->buffer, 1);
...
//  if(!data->geometry.bytes_per_sector)
//      data->geometry.bytes_per_sector=512;
